...
if __name__ == "__main__":
    ...
    # Example: Running specific configuration
    target_state_name = "State 1 (Very Easy)" # Change this to test different states
    
    print(f"--- Experiment: {target_state_name} ---")
    initial_state = INITIAL_STATES[target_state_name]
    
    # Setup Problem
    problem = Problem(initial_state)
    
    # 1. Greedy with h1
    # fringe = GreedyFringe(h1_misplaced_tiles)
    
    # 2. Greedy with h2
    # fringe = GreedyFringe(h2_manhattan_distance)
    
    # 3. A* with h1
    # fringe = AStarFringe(h1_misplaced_tiles)
    
    # 4. A* with h2
    fringe = AStarFringe(h2_manhattan_distance)
    
    start_time = time.time()
    path, cost, expanded, status = tree_search(problem, fringe)
    end_time = time.time()
    
    print(f"Algorithm: {fringe.__class__.__name__}")
    print(f"Heuristic: {fringe.heuristic_func.__name__}")
